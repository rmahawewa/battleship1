...
function getValidity(allBoardBlocks,isHorizontal,startIndex,ship){
    ...
    //handle vertical
    startIndex <= width * width - ship.length ? startIndex : startIndex - ship.length * width + width
...
function startGame(){
    if(playerTurn === undefined){
        if(optionContainer.children.length != 0){
            ...
            allBoardBlocks.forEach(black => block.addEventListener('click', handleClick))
            playerTurn = true
...
function checkScore(user, userHits, userSunkShips){
    ...
    if(computerSunkShips.length === 5){
        infoDisplay.textContent = 'You sunk all the computers ships. COMPUTER WON!'
        gameOver = true
    }
